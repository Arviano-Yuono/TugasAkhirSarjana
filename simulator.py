...
class Simulator(nn.Module):
    ...
    def update_node_attr(self, frames, types:torch.Tensor):
        ...
        one_hot = torch.nn.functional.one_hot(cell_type, 4).int()
    ...
    def forward(self, graph:Data, velocity_sequence_noise = None):
        ...
        if self.training:
            if velocity_sequence_noise is None:
                velocity_sequence_noise = torch.zeros(size = graph.x[:, 1:3].shape).to(self.device)
            ...
            node_attr = self.update_node_attr(noised_frames, cell_type)
